...
export class FakeSnippetOperations implements SnippetOperations {
  ...
  getUserFriends(name: string = "", page: number = 0, pageSize: number = 10): Promise<PaginatedUsers> {
    return new Promise(resolve => {
      setTimeout(() => resolve(this.fakeStore.getUserFriends(name,page-1,pageSize)), DELAY)
    })
  }
  ...
  postTestCase(snippetId: string, testCase: TestCase): Promise<TestCase> {
    return new Promise(resolve => {
      setTimeout(() => resolve(this.fakeStore.postTestCase(snippetId, testCase)), DELAY)
    })
  }
